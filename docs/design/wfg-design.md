...
背景条数由 `background` 速率决定，注入量**不再从配额推导**（消解"改背景速率会改变注入量"）。
完全分离（总条数 = 背景 + 注入，背景不被注入挤压）见 §7.2 的未落地项。
...
- 改背景速率**不会**改变注入条数；
- 写错数量不再静默产出 0 条（配额不足时由断言/VN21 暴露）。
...
| 码 | 触发 | 消息模板 |
|---|---|---|
| INJ1 | `hit` 实体未产出告警 | `hit 用例第 %d 个实体（%s=%s）不会触发规则 <r>：<原因>` |
| INJ2 | `near_miss`/`miss` 实体产出了告警 | `%s 用例第 %d 个实体（%s=%s）会触发规则 <r>：命中 %s 路径（%s）` |
