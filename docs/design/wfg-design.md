...
已落地的扩展：**P1 实体分布**（热点 Zipf + 新老比例）→ §10；**P2 跨流注入**（join 家族三形态）
→ §9。

未落地的需求**集中登记在 §11**（每条给出现状 / 依据 / 落地要点 / 开放问题），本节不再复述，
避免两处说法漂移：

| 原优先级 | 需求 | 登记 |
|---|---|---|
| P2 | 速率波形（`wave` / `burst` / `timeline` 语义） | §11 **R3** |
| P2 | 场景矩阵（同一场景多参数批量运行） | §11 **R1** |
| — | 分片 / 多实例下的断言口径（§7.2 未决） | §11 **R2** |
| — | 发布口径（alpha channel 版本推进） | §11 **R4** |
| P3 | 模板化 · 基线对比 · 报告输出 | §11 **R1**（矩阵配报告）/ **R5** 备注 |
...
原 §7.3 P2 的「跨流注入」：让 `.wfg` 能造出**跨流配对**的数据。驱动场景是 join 家族
...
原 §7.3 P1。背景事件默认**每个字段每条现随机**（`stream_gen.rs` 的字段循环直接
...
集中登记**尚未落地**的需求，每条给出现状、依据（需求强度）、落地要点与开放问题。已落地的设计
见 §8–§10。口径：**这不是 bug 清单**，而是"要不要做 / 怎么做"的决策输入——`§7.3` 只做索引。

### R1 场景矩阵：同一场景的多参数批量运行

| 项 | 内容 |
|---|---|
| 需求 | 同一份 `.wfg` 以多组参数（速率 / 实体数 / 分布 / 窗口…）批量跑，并汇总对比 |
| 现状 | **既无语法也无 CLI**：`<>` 只消费 `seed` 与 `duration`（§2.1），注解键由 VN29 白名单守着；实际由外部 shell 兜着（`wf-examples/performance/nexmark_pk/bench.sh`、`rule_scale_test/sweep.sh`） |
| 依据 | **中**——需求真实（perf 目录在手工 sweep），但当前由脚本层满足 |
| 落地要点 | 参数化（占位 / 参数表）→ 矩阵枚举 → 每格独立输出目录 → 汇总对比 |
| 建议 | **与 P3 的「基线对比 / 报告输出」一起做**：矩阵单独做只是"重复跑"，配报告才划算 |
| 开放问题 | 参数写在哪——`--matrix` CLI 参数，还是场景内声明？ |
...
| 项 | 内容 |
|---|---|
| 需求 | 分支上的版本与发布 channel 的元数据一致 |
| 现状 | `alpha` 分支 `version.txt = 0.7.0`、CHANGELOG 顶部 `[0.7.0]`，但 `updates/alpha/manifest.json` 仍指向 `0.6.3-alpha`（`updates/beta` = 0.6.2-beta、`updates/stable` = 0.1.29） |
| 依据 | 发布时处理（属发布流程，不属 DSL 能力） |
| 落地要点 | 走一次发布：构建产物 → 写 sha256 → 更新 `updates/alpha/manifest.json` + 新增 `updates/alpha/versions/v0.7.0-alpha.json` |
| 开放问题 | alpha 是否承接 v0.7.0（含 `.wfg` 破坏性变更），还是另择 channel？ |

### R5 工程约定：功能分支策略

| 项 | 内容 |
|---|---|
| 现状 | 近几轮改动直接提交 `alpha` 并推送（`feats/wfg` 已并入 alpha） |
| 开放问题 | 后续功能是否统一走 `feats/*` 分支再合入 `alpha`？（非功能需求，登记备查） |

> P3 的「模板化：`template/param` 复用场景片段」尚未展开为需求记录——它与 R1（参数化）高度重叠，
> 待 R1 定案后一并细化。
