...
- **Two scoping bugs on the expectation side for chained rules (they affected `.wfg` corpora comparison)**: the engine loads the whole `.wfl` (every rule in the file runs) and L3 compares the **full alert stream**; but the oracle (a) computed the "intermediate windows" set filtered by `injected_rules`, so when a corpus targeted only the upstream rule the window consumed downstream was written into the expectations as a **final alert** (chained queries were permanently `missing`), and (b) evaluated only the **injected** rules, so a downstream rule's sink-visible alerts showed up as `unexpected`. Both are now computed over **all loaded rules** (matching the engine's `collect_intermediate_targets` and "whole file runs"), and `wfgen verify` drops expectation rows flagged `intermediate: true` (`emit()` returns early for intermediate pipeline outputs — it only relays them into the window for downstream rules, never to sinks; no sink configuration can change that).
- **`wfgen verify`'s matching no longer parses timestamps inside its inner loop**: `greedy_match` used to call `parse_time_approx` (a chrono ISO parse) for *every* (expected × actual) pair, so the cost was ≈ Σ(group_size²) × one date parse. Corpora with a **hot entity** (thousands of alerts on the same `entity_id`; the benchmark q21 corpus has one group of 8050) pushed it to **5.4s** (~80% of the query's wall time). Timestamps are now parsed once per side and the inner loop is a plain f64 comparison: the same corpus takes **0.28s** (19×), the 22-query verification suite goes **27s → 22s**, and all 22 comparison reports are **byte-identical** (matching results are equivalent). A hot-group case was added as a regression guard.
- **stats close semantics in the oracle aligned with the engine**: the `stats<…>` close `entity_id` used to be the bucket key's Debug text (`Int(0)` / `Pair(…)` / `Empty`); it now **evaluates the rule's `entity(...)`** (constant → literal, field → grouping-key column value), and `origin` changed from `close` to **`close:timeout`** (the engine's stats close is hard-wired to `CloseReason::Timeout`). Any mismatch here makes every stats alert un-pairable.
