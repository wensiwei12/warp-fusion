...
pub(super) fn fetch_remote_tags(repo: &Repository, repo_url: &str) -> Result<(), String> {
...
fn clear_local_release_tags(repo: &Repository) -> Result<(), String> {
    let refs = repo
...
fn is_first_initialization(work_root: &Path, group: Option<RemoteGroup>) -> Result<bool, String> {
    match group {
        Some(g) => {
...
fn resolve_latest_released_target(repo: &Repository) -> Result<Option<ResolvedTag>, String> {
    let names = repo
        .tag_names(None)
...
fn resolve_remote_head_target(repo: &Repository) -> Result<ResolvedTag, String> {
    let head = repo
...
fn origin_head_not_symbolic_err() -> String {
    "origin HEAD is not a symbolic ref".to_string()
}

pub(super) fn checkout_commit(repo: &Repository, commit_id: Oid, tag: &str) -> Result<(), String> {
    let commit = repo
