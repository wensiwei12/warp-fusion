...
`.wfg` 场景 DSL 的注入语法重写（**破坏性**，现有场景需改写），并补上生成期硬断言；引擎对齐 wp-reactor **2.3.0**、连接器对齐 wp-connectors **0.21.0**。
...
- **admin API 的超限请求体稳定返回 413**（此前偶发「连接被重置」）：同一个超限 body 有时拿到 `413 payload_too_large`、有时被重置连接（客户端仍在校验前写入时报错时机不定）；现已稳定返回 413，且连接可复用。
- **`.wfg` 校验与生成的口径一致性**（五处，此前要么直接 panic，要么「`wfgen lint` 说 OK、`wfgen gen` 才失败」，要么静默改掉生成的数据）：
  ...
  - **join 块里写驱动侧实体键字段现报 VN12**：此前只拦了右窗连接键，写驱动侧那些字段同样会被静默替换；
...
- **`wfgen verify` 不再把「空对空」算成通过**：两侧都是 0 条时报 `pass`（「期望生成 / 断言根本没跑」也会亮绿灯）；现默认判失败，要接受空输入须显式 `--allow-empty`（`wfl verify` 同名旗标）。
- **`--no-expect` 不再静默关掉注入断言**：断言吃的是期望那份评估结果，关掉期望等于断言没跑（数据照旧产出）；现出声警告，并清掉上一次遗留的期望文件——陈旧期望会让对拍拿旧期望比新数据、看起来却有证据。
- **`use(...)` 里写实体键字段一律拦下（VN12）**：写进去的值会被静默换成生成器算的实体值，数据里不是写的那个数、`hit` / `near_miss` 断言也会指错实体。此前只覆盖「用例头显式写字段」与「join 连接键」，主路径（从规则推断实体字段）反而漏了。
- **注入实体字段口径统一**（几处「写了不生效 / 不写就断言不到」）：显式 `key` 映射规则写来源字段；`entity(...)` 的单字段总是写进事件（join-then-key 规则此前只「空跑」不断言）；join 驱动侧连接键由生成器自动对齐，不再需要手写 `use(...)` 去凑；VN12 / VN27 按同一口径同步。
- **背景不再与注入实体撞值**：背景噪声偶然取得注入实体的键值时会被算到该实体头上（`near_miss` / `miss` 的「必不报警」变成概率性）；现这类字段的背景取值与注入区间分区（没声明 `entity … zipf(...)` 池也生效）。
- **`replay` 的记录时间必须有序**：出现回退直接报错（VN26），不重排（那是静默改用户数据）。
- **校验口径收紧**：新增 **VN33**（用例的 stream 必须是规则绑定窗）、**VN35**（时间字段不能由 `use` / `without` / join 覆盖，否则会造成双时间轴）；VN24 按「该 stream 上」的事件步骤数、VN30 只认 join 首条件且要求唯一匹配。
...
- **「能编译、但永远不生效」的写法改为编译期报错**：集合 / 统计函数（`first` / `collect_*` / `stddev` / `percentile` 等）与 `window.has(...)` / `baseline(...)` 写在不支持的位置、无限定的 `has(...)`、`stats` 桶键写成表达式 —— 这些此前能过编译、运行期静默求值为空（规则不触发、告警缺失且全程无报错），现在直接报错并提示可用位置。
- **大整数不再被量化**：`|v| >= 2^53`（纳秒时间戳、雪花 ID 等）此前经值层往返会丢精度，使「相差 <128ns」的区间比较随机翻转、同刻跨流配对丢约一半命中；现全程由精确整数承载（未声明类型的整数字段也由浮点输出改为整数）。
- 修掉几处**静默丢数据 / 静默回绕**的路径：`arrow_framed` 文件源单文件超过 1 批后不再丢批；小数形式时间戳（`"1700000000123.0"`）不再偏 64ns；接收侧不支持的源类型组合不再让整个投影作废（改为空值列 + WARN）；时长字面量溢出（如巨大的 `…d`）改为报错。
